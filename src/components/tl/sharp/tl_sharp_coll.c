...
/**
 * 在 allreduce 之后选择数据，以此来实现 reduce scatter
 */
ucc_status_t ucc_tl_sharp_reduce_scatter_start(ucc_coll_task_t *coll_task)
{
    ...
    size_t                        recv_count = args->dst.info.count;
    ucc_datatype_t                dt         = args->dst.info.datatype;
    struct sharp_coll_reduce_spec reduce_spec;
    enum sharp_datatype           sharp_type;
    enum sharp_reduce_op          op_type;
    ...
    int                           ret;
    int                           local_rank = TASK_LOCAL_RANK(coll_task);
    int                           global_ranks = TASK_GLOBALRANKs(coll_task);
    ...
    recv_data_size  = ucc_dt_size(dt) * recv_count;
    ...
    ucc_tl_sharp_mem_register(TASK_CTX(task), team, (void*)recv_buf, send_data_size,
                              &task->allreduce.r_mem_h);
    ...
    reduce_spec.sbuf_desc.type              = SHARP_DATA_BUFFER;
    reduce_spec.rbuf_desc.buffer.ptr        = (void*) recv_buf;
    ...
    int unit_size = ucc_dt_size(dt);
    uint8_t* buffer = (uint8_t*)args->dst.info.buffer;
    for (int i = local_rank, j = 0; i < send_count; i += global_ranks, j += unit_size) {
        memcpy(buffer + j, recv_buf + i * unit_size, unit_size);
    }
